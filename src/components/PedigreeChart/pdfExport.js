...
export const exportPedigreeToPDF = async (
  ...
  try {
    ...
    try {
      ...
      unspecifiedImage = await loadImageAsBase64("/assests/unspefied.png", false, 30);
    } catch (error) {
    ...
    const loadCountryFlag = async (countryCode) => {
      try {
        ...
        const flagImage = await loadImageAsBase64(flagUrl, false, 24);
        
        // Store in cache
        flagCache[countryCode] = flagImage;
        return flagImage;
      } catch (error) {
...
const PedigreeExportButton = ({
  ...
  const handleExport = useCallback(async () => {
    try {
      await exportPedigreeToPDF(nodes, edges, pedigreeData, profileData, generations);
    } catch (error) {
